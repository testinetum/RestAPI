...
public class Class2 {
    ...
    public void testUiAndApi() {
        ...
        Response response = RestAssured.given().when().get("/pet/123"); 
        ...
        SoftAssert.assertEquals(response.jsonPath().getString("name"), "doggie", "Name does not match");
        SoftAssert.assertEquals(response.jsonPath().getString("status"), "available", "Status does not match");

        // Validate category
        SoftAssert.assertEquals(response.jsonPath().getInt("category.id"), 0, "Category ID does not match");
        SoftAssert.assertEquals(response.jsonPath().getString("category.name"), "string", "Category name does not match");
